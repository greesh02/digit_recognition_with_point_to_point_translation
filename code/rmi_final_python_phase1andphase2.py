...
transform = transforms.Compose([transforms.Resize((28, 28)), transforms.ToTensor(), transforms.Normalize((0.5,), (
    0.5,))])  # .ToTensor() converts numpy array (0 - 255) to float Tensor (0 - 1)
...
class LeNet(nn.Module):
    ...
    def forward(self, X):
        ...
        x = self.dropout1(x)
        x = self.fc2(
            x)  # raw output called as score -- to avoid to pass in values very close to zero or 1 (in loss function)
